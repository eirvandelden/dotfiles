...
| `review` | One skill, two backends — default: a fresh reviewer in a herdr pane (today's `/review`); `review here`: the `reviewer` agent forked in-session, also the fallback outside herdr. Report-only: reads `REVIEW.md` + `plan.md` + `spec.md` + diff; compliance pass includes criterion→test coverage and no weakened tests; writes findings to `docs/changes/<slug>/review.md` (the only file it may write), one dated round per run, and commits it; never edits code | Claude: subagent; Codex: fresh session or Codex subagent if the docs confirm one exists |
| `finish` | One skill, scope from the remote. Both: confirms `review.md` is fresh and closed; fills the repo PR template from `intent.md`/`plan.md` (Summary/Why ← intent; Implementation ← plan; Testing ← plan Proof); ADR for cross-application changes (work only); deletes the folder; commits; pushes; creates the PR or updates its body if one already exists; opens it in the browser. Work only, after opening: runs the private after-push script, which marks the PR ready and opens it in the work browser profile so Etienne sets the team's review status by hand (decided 2026-09-22: no board automation); requests reviewers, offering the last-confirmed list across sessions. Assumes review already happened. Runs only on explicit command | same; board mechanism in a private after-push script |
...
- `review` absorbs today's pane-only skill: the default runs the reviewer in a herdr pane via `start-review.sh` — a fresh agent with an empty context, which is the point of a review; `review here` runs the `reviewer` agent in-session (`context: fork`) for terminals without herdr. Outside herdr (`HERDR_ENV` unset) the skill falls back to `here` and says so. Both append a round to `review.md` and commit it. One skill, two backends.
