...
| `finish` | One skill, scope from the remote. Both: confirms `review.md` is fresh and closed; deletes the folder; commits. Work only, before the delete: fills the repo PR template from `intent.md`/`plan.md` (Summary/Why ← intent; Implementation ← plan; Testing ← plan Proof); ADR for cross-application changes. Work only, after: pushes; runs the private after-push script, which marks the PR ready and opens it in the work browser profile so Etienne sets the team's review status by hand (decided 2026-09-22: no board automation); requests reviewers. Assumes review already happened. Runs only on explicit command | same; board mechanism in a private after-push script |
