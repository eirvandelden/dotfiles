...
Part of the change in `docs/changes/ai-native-workflow/` (read `intent.md`, `spec.md`, `plan.md`, `habits.md` first). Repository: `~/Developer/dotfiles`. Requires phase 2 merged (the artifact skills replace what the plugins did). Work in a worktree, PR against `origin`.
