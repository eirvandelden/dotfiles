...
- **ADR location in work repos**: neither work application has an ADR folder today. The work-specifics file proposes one; Etienne confirms with the team before the first ADR lands.
...
3. **`prepare-for-team` skill** (`disable-model-invocation: true`; runs only on explicit command; supports `--dry-run` that prints every step and changes nothing). Steps:
   ...
   - ADR question (work only): "Does this change affect another application — API contract, published event, shared schema, anything another team consumes?" On yes: write the ADR from `intent.md` + `spec.md` into the location named by the after-push script's companion config (see step 4); on no: continue.
