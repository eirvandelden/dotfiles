...
0. **Confirm the scan path once more, cheaply.** `codex --version`; then from `~`, `codex exec --sandbox read-only 'From your skill loader alone, is a skill named superpowers-ruby:brainstorming registered, and one named worktree-first? Two lines, yes/no, with the path the loader reports.'` Expected: brainstorming yes under `~/.agents/skills/superpowers/…`, `worktree-first` no. Paste the answer into the PR description. If `worktree-first` is reported yes with a `~/.codex/skills` path, stop and report — the premise changed.
...
- **Etienne, by hand, after merge:** from the dotfiles root, `stow -R --no-folding codex` (removes the now-orphaned `~/.codex/skills/*` links that came from this repo), then `stow --no-folding agents` and `stow -R --no-folding claude`. Then from `~`: `codex exec --sandbox read-only 'From your skill loader alone: is a skill named worktree-first registered? Path?'` must answer yes with a path under `~/.agents/skills/`. In a fresh `claude` session confirm `/handoff` is listed. Report back if either is missing. The dotfiles-work half (its own `agents` package for `fizzy-sync`) is in `~/Developer/dotfiles-work/docs/changes/ai-native-workflow/work-specifics.md`, "Phase 1".
