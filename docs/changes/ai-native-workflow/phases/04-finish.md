...
3. **`finish` skill** (`disable-model-invocation: true`; runs only on explicit command; `--dry-run` prints every step and changes nothing). Steps, stopping at the first failure:
   - Preconditions, both scopes: `review-report-fresh` exits 0 (else: "run `/review-branch` first"); `review.md` has no open finding without a later round (else list them: "close or dismiss these first"); working tree clean; scope resolved by `change-scope`.
