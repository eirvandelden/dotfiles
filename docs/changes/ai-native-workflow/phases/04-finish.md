...
1. **RED — PR body filler.** `claude/.claude/skills/finish/scripts/fill-pr-template` (Ruby). Input: template path, `intent.md`, `plan.md`. Output: the template with sections filled. A heading matches the summary category when it contains "summary" or "description", or when it *starts* with "What does", "What changed", or "What is this" (case-insensitive); a heading containing "type" never matches summary (decided 2026-09-23, Round 2 review: "what" alone was too broad and swallowed "What type of change is this?"). Each category keyword is matched with a leading `\b` (decided 2026-09-23, Round 3 review: a bare substring matched "test" inside "Latest" and "how" inside "showcase"; the leading boundary still lets a heading like "Testing" match as a prefix). /\bwhy|\bmotivation/i gets the Problem; /\bimplementation|\bhow|\bdetails/i gets the plan's Files that change + Order of work; /\btest|\bverify/i gets the plan's Proof. Independently of category, a section whose body contains a checkbox list (`- [ ]` or `- [x]`) is a choice list and is never filled, whatever its heading matches. Unknown headings keep their template text; a matched heading's section runs to the next heading of the same or higher level, so its sub-headings are replaced with it, not filled again — except a sub-section that is a choice list, which stops that absorption and is kept, along with everything after it, as is (decided 2026-09-23, Round 3 review: absorption was dropping a checkbox list nested under a matched heading). Whichever of the four categories ends up with no heading filled anywhere — none matched, or the one that did is a choice list — is appended at the end of the body under its own heading, `## Summary` / `## Why` / `## Implementation` / `## Proof`, in that order; a template with no matching headings, or no template at all, gets all four appended this way, replacing the old single `## Context` block (decided 2026-09-23, Round 3 review). Test `test/fill_pr_template_test.rb` with two fixture templates (a checkbox-style one and a plain one) and the no-template case. Run: fails. Write.
...
3. **`finish` skill** (`disable-model-invocation: true`; runs only on explicit command; `--dry-run` prints every step and changes nothing, including no `gh pr create`). Steps, stopping at the first failure:
   ...
   - **Both**: find the PR template (`.github/PULL_REQUEST_TEMPLATE.md`, `.github/pull_request_template.md`, `PULL_REQUEST_TEMPLATE.md`, or `docs/pull_request_template.md`); run `fill-pr-template`; show the result. No template found: `fill-pr-template` still produces the four `## Summary` / `## Why` / `## Implementation` / `## Proof` sections from `intent.md` and `plan.md` alone.
