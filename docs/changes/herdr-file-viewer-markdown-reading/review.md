...
- [ ] Important: Six of the spec's acceptance criteria (the five viewer checks and the stow-result inspection) are still unexercised. The plan defers them until an approved restow of `herdr`, which has not happened: `~/.config/herdr/README.md` and `~/.config/herdr/plugins/config/herdr-file-viewer/config.toml` do not exist. The branch should not be called done until the restow is approved, `herdr server reload-config` is run, and each check is recorded. — `docs/changes/herdr-file-viewer-markdown-reading/plan.md:82` → open: cannot close on this branch. Stow sources from the main checkout, which does not carry these files until the branch merges. Recorded as the remaining work rather than dismissed.
- [x] Important: The spec promises the failure "names that file and the `~/README.md` it would become". The fixture test only asserts the stray list `["untidy/README.md"]`; the `-> ~/README.md` message is built only in the real-repository test, which passes, so that message is never exercised by any test. — `test/stow_package_roots_test.rb:23` → fixed: the message moved into `stray_report`, and `test_the_rejection_names_the_file_and_where_it_would_land` asserts it names both the stray and `~/README.md`.
- [x] Nit: `Dir.children` never returns `.` or `..`, so `- [ ".", ".." ]` removes nothing. — `test/stow_package_roots_test.rb:53` → fixed: removed.
- [x] Nit: A package declared in `packages.conf` with no directory on disk is skipped silently (`next [] unless File.directory?`), so a misspelled package name passes the guard. Maybe intended; if so, a one-line reason would help. — `test/stow_package_roots_test.rb:51` → fixed: intended, and the reason is now in the code — a declared package with no directory is packages.conf's bug, and stow itself fails on it at install time.
- [x] Nit: "worktrees created with `git worktree add` are invisible to it" reads as if the viewer cannot see them, but the next paragraph says `W` lists them. What is missing is a herdr workspace for them, not visibility to the viewer. — `herdr/.config/herdr/README.md:30` → fixed: reworded to say no pane is ever in one, and that the viewer can still reach them.
- [x] Nit: The prose refers to `y` (copy path from a pin) and `i` (reveal ignored entries), but the "Keys worth knowing" table lists neither. — `herdr/.config/herdr/README.md:36` → fixed: both added to the table.
- [x] Nit: `project-dictionary.txt` changes but is not in the plan's "Files that change" table. The words are legitimate (`Haije` from `intent.md`, `smarzban` from the install command, `hjkl`/`solunized` from existing lines of `config.toml` that cspell now scans), so this is a plan-accuracy note only. — `project-dictionary.txt:227` → fixed: added to the plan's "Files that change" table.
